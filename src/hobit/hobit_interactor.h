...
#ifndef HOBIT_INTERACTOR_H
#define HOBIT_INTERACTOR_H
...
/** This class is neccessary for every kind of user Interaction.
  ...
  * the security medium.
  * This is just a wrapper class for the Qt-compatible class GuiInteractor.
  * This avoids multiple inheritance, which is incompatible to HBCI and Qt,
  * because both must be the first class to be inherited from.
  *
  * @author Marcel Mulch
  */
class HoBIT_Interactor : public HBCI_Interactor  {
public: 
  /**
  * base constructor
  * creates a new interactor
  */
  HoBIT_Interactor( QWidget *parent, HBCI::API *api);
  ~HoBIT_Interactor();
...
/**
   ...
   * Please note that this pointer may be invalid.
   */
  virtual bool msgInsertCorrectMediumOrAbort(HBCI::Pointer<HBCI::User> user, MediumType type);
...
private:
   /** The Qt-compatible class that runs the dialogs
   */
   GuiInteractor * _Gui;
  ...
  /** The pin dialog to be shown */
  DlgEnterPinKeypad* _enterPinKeypad; 
  ...
  */
  bool _Wait;
  /**Counter for PIN input trials. Causes warning message to be sent.
  * Independent of the banking engine.
  */
  int _PinInputTrials;
};
